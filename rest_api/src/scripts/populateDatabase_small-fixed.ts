...
const populateCarListings = async () => {
  ...
  // get all car images
  const carImages = JSON.parse(fs.readFileSync('json/carimgs.json', 'utf-8'));
  ...
  // Create a progress bar to track the progress
  const carListingPbar = createPBar(100);
...
const exportAll = async () => {
  ...
  await fs.promises.writeFile('json/watchlists.json', json);
};
...
const populateDatabase = async () => {
  // If there is no data available locally, use this method to generate small datasets and export data to JSON files.
  // Once data preparation is complete, comment out this block and use the "importAll" function to import data.
  // await deleteAll();
